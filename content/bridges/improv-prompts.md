---
title: "Improv Prompts: 110 Scene Starters and How to Use Them"
description: "A hundred and ten prompts you can use tonight — relationships, first lines, locations, situations — and why the zany ones make worse scenes."
target_keywords:
  - { keyword: "improv prompts", volume: 700, difficulty: 0, traffic_potential: 1700 }
  - { keyword: "improv ideas", volume: 300 }
  - { keyword: "improv scenarios", volume: 150 }
  - { keyword: "improv topics", volume: 90 }
  - { keyword: "improv scene ideas", volume: 50 }
  - { keyword: "improv situations", volume: 20 }
entry_atoms: [suggestion, base-reality, initiation, obvious-choice, offers]
entry_path: beginner-foundations
primary_problem: "you need something to start a scene with and everything you think of is either boring or too clever"
primary_cta_type: exercise
primary_cta_target: first-line-drill
secondary_cta_target: beginner-foundations
status: draft
created: "2026-08-22"
updated: "2026-08-22"
---

# Improv Prompts: 110 Scene Starters and How to Use Them

Most lists of improv prompts are lists of premises — a dentist who is afraid of teeth, a wedding where nobody knows the bride. They are fun to read and they make bad scenes, because a premise is a joke that has already happened. The players spend the next three minutes illustrating it.

What follows is a hundred and ten prompts organised by what they actually give you, plus the part the lists leave out: what to do with one once you have it.

## What Makes a Prompt Work

`suggestion` is the craft of taking audience input and using it. The criteria in it are worth stating plainly, because they cut most prompt lists in half.

**Specific beats general.** "A place" is nothing. "Laundromat" is something. "Your grandmother's attic" is better than both, because it arrives with smell, light, and a dead person's belongings.

**Personal beats clever.** "What stressed you out this week?" produces better material than "give me a word," because the answer is attached to something real. Suggestions drawn from experience carry emotional freight that invented ones do not.

**A prompt is not a punchline.** A suggestion that arrives pre-loaded with a joke constrains rather than liberates — the audience already knows what the scene is, so the only remaining job is to deliver it. That is a recital, not an improvisation.

**Evocative, not prescriptive.** "Thanksgiving" opens a hundred doors. "A guy who trips carrying the turkey" opens one and stands in it.

## Relationship Prompts

The most useful kind, and the kind nobody lists. `base-reality` needs four things — character, relationship, objective, where — and relationship is the one that generates the other three. Give two players a relationship with something already in it and the scene starts itself.

- Two siblings clearing out their mother's house
- A driving instructor and a student who is forty
- Best friends, one of whom just got the job the other applied for
- A couple on the drive home from a party
- Two people who used to be in a band together
- A doctor and a patient who went to school together
- Neighbours who share a fence and disagree about it
- A parent helping an adult child move out again
- Two people who matched online and have now met
- A manager and the person they are about to promote
- Old friends, one of whom has become religious
- A landlord and a tenant who is three weeks late
- Two colleagues stuck in a lift
- A grandparent teaching a grandchild to drive
- Someone and their ex's new partner
- Two people who were briefly famous together
- A coach and the player they are dropping
- Roommates dividing what they bought together
- A translator and the person they are translating for
- Someone visiting a friend in hospital
- Two people who have been on one date before
- A parent and the teacher who called them in
- Siblings deciding who takes the dog
- A priest and someone who has stopped coming
- Two people who used to work night shifts together
- A tailor and a customer for a suit they will wear once
- Someone and the person who trained them, years later
- Two people who survived the same accident

## First Lines You Can Open With

`initiation` is the craft of starting a scene with one specific choice that implies the world rather than explaining it. These are lines to say, not situations to build toward. Say one and find out what it means.

- "She has your eyes."
- "You said you'd stopped."
- "Everyone's outside. They're waiting for you."
- "I found the receipts."
- "Don't tell Mum I let you do this."
- "This is the last time I'm asking."
- "It doesn't look like the photo."
- "I've been standing here for an hour."
- "You're the only one who wrote back."
- "Whose car is that?"
- "I brought the thing you asked for."
- "Nobody's used this room since."
- "I'm going to need you to sit down."
- "You always sit on my side."
- "They said you'd know what to do."
- "It was cheaper than the alternative."
- "You've done something to your hair."
- "I'm not signing that."
- "Third time this month."
- "I kept your chair."
- "Your father would have hated this."
- "Put it back exactly where it was."
- "I told them you were coming."
- "We agreed we wouldn't talk about it."

`obvious-choice` matters more here than anywhere. The first line that occurs to you is connected to the scene. The second one is connected to your ego.

## Locations Worth Playing

A where is the most overlooked element of `base-reality` and the cheapest way to make a scene specific. These reward `space-work` — they have objects in them, and objects give you something to do while you find out what the scene is.

- A laundromat at two in the morning
- The queue at a passport office
- A caravan in the rain
- Backstage at a school play
- A car boot sale, packing up
- The waiting room of a fertility clinic
- A hotel breakfast buffet
- An empty office on Christmas Eve
- A cable car that has stopped
- The bit of the garden centre nobody goes to
- A funeral director's showroom
- A launderette that is also a post office
- The car park after the wedding
- A campsite where it will not stop raining
- Someone's childhood bedroom, unchanged
- A ferry crossing in bad weather
- The lost property room at a station
- A shed at the bottom of a garden
- An airport gate after the last flight
- A lighthouse, being decommissioned

## Situations With Something Already Wrong

These give you an objective and a problem without prescribing the joke. The comedy comes from playing them straight.

- Someone has brought the wrong ashes
- The cake says the wrong name
- One of you has already spent the money
- The house sale falls through on the doorstep
- A gift is being returned to the person who gave it
- The dog has been to the vet and only one of you knows why
- Someone has been coming to this class for a year under a false name
- The recording was left running
- Two people have been sent the same apology letter
- The inheritance is a boat
- Somebody sold the piano
- The photograph has someone in it who should not be
- A speech has to be given and the notes are gone
- One of you has been fired and has not said
- The neighbours have built something over the line
- The ring is the wrong size and it is too late

## Questions to Ask an Audience

If you are running a show and want better material than "banana," ask for the thing rather than the word. `suggestion` is explicit about this: personal input yields richer scenes than abstract input.

- What did you do on your last day off?
- What's something you've kept that you should have thrown away?
- What's the worst job you've had?
- Where did you go on holiday as a child?
- What's a rule your family had?
- What's something you're avoiding this week?
- What did you want to be at fourteen?
- What's the last thing you argued about?
- What's in your kitchen drawer?
- Name a place you've driven past a hundred times and never stopped.
- What's a smell that takes you somewhere?
- What was your first job's uniform?

## Prompts for Teachers and Warm-Ups

For a class or a corporate room, the prompt has a second job: it has to be safe enough that nobody has to be funny to succeed. `safety-in-the-room` is the precondition, and these are chosen to be answerable by anyone.

- Two people assembling something with no instructions
- A person teaching another person something physical
- Two people who agree about everything
- A scene where nobody may ask a question
- Two people waiting for the same appointment
- One person arriving late to something small
- Two people looking at the same object and seeing different things
- A scene played entirely in one corner of the room
- Two people who have to whisper
- Someone showing another person around a place they love

## Why the Absurd Ones Backfire

The most common request is for funny improv scenarios — a talking fridge, a vampire dentist, aliens at a barbecue. They read well and they play badly, and the reason is structural rather than a matter of taste.

`base-reality` should not be funny, weird, or absurd. When it is, the grounding step has been skipped and there is nothing for the unusual thing to be unusual *against*. A scene where everything is strange has no contrast, so nothing can escalate — every heightening move lands at the same volume as the one before.

There is also a bandwidth cost. Every offer has a processing cost for your partner. "Happy birthday" costs nothing. "I'm a cyborg from the future here to steal your toaster" demands five minutes of exposition before anyone can play. Spend the shared attention on each other, not on comprehension.

The workable version: take the absurd prompt and play the people in it completely straight. A talking fridge is a bad scene. Two flatmates having the argument they have been avoiding for a year, while a fridge listens, is a scene — and the fridge is funnier for being ignored.

## How to Use a Prompt Without Being Imprisoned by It

A prompt is a launching point, not a subject. Will Hines puts the show-craft version plainly: the suggestion is there to prove it's improvised, and after that its job is done.

**Free-associate rather than depict.** "Dentist" leads to cavity, to hollow, to empty, to loneliness. The scene can be about a person who has stopped answering their friends, sparked by "dentist" and never mentioning teeth.

**Take the second thing it makes you feel, not the first thing it makes you think.** The first thought is usually a place or a job. The feeling underneath it is what the scene is made of.

**Drop it the moment something better arrives.** If your partner makes an offer that has nothing to do with the prompt, take the offer. `offers` outrank suggestions — the prompt came from a stranger, and your partner is in the scene with you.

**Do not check back.** Scenes die when a player returns to the prompt to make sure they are still obeying it. Nobody in the audience is auditing you.

## Practise the Starting Part

The hardest part of any prompt is the first three seconds. `first-line-drill` isolates exactly that: players line up, each steps out and delivers a single opening line, and nothing else happens. No scene, no rescue, no chance to fix it — just the threshold, over and over, until starting stops being the difficult bit.

For the rest of the craft these prompts feed into, [improv games](/improv-games) covers what to play, [rules of improv](/rules-of-improv) covers the principles underneath, and [how to get better at improv](/how-to-get-better-at-improv) covers what to work on next.
