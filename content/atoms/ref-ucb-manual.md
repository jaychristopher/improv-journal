...
links:
  ...
  - { id: if-this-then-what, relation: illustrates }
  - { id: heightening, relation: illustrates }
  - { id: base-reality, relation: illustrates }
  - { id: straight-man, relation: illustrates }
  - { id: mapping, relation: illustrates }
...
The most systematic book ever written about improv. Where *Truth in Comedy* is a manifesto, this is a curriculum: definitions, worked examples, and a stated method for nearly every situation a two-person scene can produce.

For the school and what it chose not to inherit, see [UCB](/traditions/ucb). This page is about the book.

## What is actually in it

It builds in one direction, from the two-person scene outward. `game-of-the-scene` is the organising idea and most of the book is machinery for finding and running one: the first unusual thing, `if-this-then-what` as the engine for exploring it, `base-reality` established first so the unusual thing has something to be unusual against, the `straight-man` who makes it legible, and `heightening` as the way it escalates.

The later half moves to ensemble work — `harold`, group scenes, `editing`, and The Movie — but the two-person material is the part the book is known for and the part that changed how improv is taught in North America.
...
The failure mode is the direct cost of the same virtue. A procedure invites you to run it, and analytical game-hunting produces scenes that are technically correct and completely dead — two people executing a pattern neither of them cares about. Will Hines, who taught at UCB for years, has argued that game is better felt than identified, and the best UCB performers tend to have absorbed the vocabulary and stopped consulting it.
