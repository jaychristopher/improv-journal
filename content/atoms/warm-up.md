---
id: warm-up
title: Warm-Up
type: definition
status: draft
tags: [definitions, fundamentals, ensemble, preparation]
links:
  - { id: presence, relation: enables }
  - { id: ensemble, relation: enables }
  - { id: trust, relation: enables }
  - { id: internal-computation, relation: contrasts }
  - { id: judgment, relation: contrasts }
  - { id: active-listening, relation: enables }
  - { id: be-brave, relation: enables }
  - { id: vulnerability, relation: enables }
  - { id: bandwidth, relation: extends }
  - { id: group-mind, relation: enables }
  - { id: ref-spolin-improvisation-for-theater, relation: extends }
  - { id: ref-attention-and-effort-kahneman, relation: extends }
  - { id: mirroring, relation: illustrates }
sources: []
created: "2026-04-05"
updated: "2026-08-24"
---

Warm-Up is the ritual that prepares performers for improvisation — not just physical warm-up but a neurological, ensemble, and psychological transition from daily-life mode to performance mode.

## Neurological purpose: lowering the self-monitor

Limb & Braun (2008) showed, in fMRI research, that during improvisation, the dorsolateral prefrontal cortex (self-monitoring, planning, evaluation) shows *reduced* activity while the medial prefrontal cortex (self-expression, internally motivated behavior) shows *increased* activity. Warm-ups accelerate this shift. Without them, performers spend the first 10-15 minutes of a show still in their "daily life brain" — monitoring, judging, planning.

## Ensemble purpose: building shared energy

Every performer arrives in a different state. Warm-ups create a shared baseline — through group activities, the ensemble literally synchronizes breathing rhythms, energy levels, and attention. Johnstone: "People who warm up together perform together. People who don't, perform next to each other."

## Physical purpose: releasing tension

Performance anxiety manifests physically: tight shoulders, shallow breathing, locked jaw. These directly impair performance — tight jaw means unclear speech; shallow breathing means quiet voice and rushed pacing. Physical warm-ups (shaking, stretching, vocal exercises) release these blocks.

## Psychological purpose: rehearsing vulnerability

Many warm-ups involve low-stakes risk-taking — making silly sounds, physical contact, looking foolish. This rehearses the vulnerability that improv requires, in a context where failure carries no consequences.

## Categories of warm-ups

- **Focus games** (Zip Zap Zop, Count to 20, Mind Meld) — train group attention
- **Energy games** (Bunny Bunny, Sound Ball, Whoosh) — build intensity, lower self-consciousness
- **Trust exercises** (Mirror Work, Blind Lead, Yes Let's) — build ensemble safety
- **Vocal warm-ups** (lip trills, tongue twisters, projection exercises) — prepare the instrument

## Spolin's foundational insight

Warm-up games aren't preliminary to the real work — they ARE the methodology. Each game creates a "point of concentration" that occupies the analytical mind while the spontaneous mind is freed to play. The games are democratic (everyone plays, no one directs), and the purpose is not to "win" but to solve the shared problem the game poses. The entire tradition of improv warm-ups descends from Spolin's theater games.

## Specific sources

Spolin, *Improvisation for the Theater*, Ch. I-II (theory) + Appendix II ("Traditional Games" for warm-up use). The "Seven Aspects of Spontaneity" section is the foundational statement of why games work. Limb & Braun, *PLoS ONE* 3(2), e1679 (2008) — dorsolateral prefrontal deactivation during improvisation. **Important caveat:** Limb & Braun studied jazz pianists, not improv actors. The transfer to theatrical improv warm-ups is an inference, not a demonstrated finding. **Counter-position:** Some experienced performers skip structured warm-ups, substituting casual conversation. The argument: ensemble connection can be achieved through familiarity and trust. The Spolin response: games are never merely preliminary — they ARE the methodology.