---
id: pacing
title: Pacing
type: technique
status: draft
tags: [techniques, intermediate, performance, rhythm, show-craft]
links:
  - { id: editing, relation: extends }
  - { id: show-dynamic, relation: extends }
  - { id: rest-beat, relation: extends }
  - { id: heightening, relation: extends }
  - { id: scene-structure, relation: extends }
  - { id: audience, relation: requires }
  - { id: reading-the-room, relation: requires }
  - { id: active-listening, relation: requires }
  - { id: be-present, relation: requires }
  - { id: commitment, relation: requires }
  - { id: ref-carrane-improv-nerd, relation: extends }
  - { id: ref-napier-improvise, relation: extends }
  - { id: ref-napier-behind-the-scenes, relation: illustrates }
sources: []
created: "2026-04-05"
updated: "2026-08-24"
---

Pacing is speed and rhythm management — within scenes and across shows. One of the skills that most separates experienced performers from intermediates.

## Micro-pacing (within a scene)

## When to slow down

- At the **top of the scene** — the first 30 seconds benefit from patience. Establishing who/what/where requires time. Rushing past the foundation means nothing to build on.
- At **emotional moments** — when something real lands, let it breathe. The temptation (especially in comedic improv) is to deflect with a joke. Resisting creates depth.
- After a **big laugh** — let the audience finish. Talking over laughter means they miss the next line and you miss the energy.
- When you're **lost** — the instinct is to speed up. The better move is almost always to slow down, look at your partner, and respond to what's actually happening.

## When to accelerate

- When the **game is established** and you're heightening — each beat can come faster as the audience catches on
- During **physical comedy** — slapstick often benefits from speed
- In the **final third of a scene** — quick acceleration to a button creates a satisfying ending

## The power of silence

Del Close: "The pause is the most powerful tool in the improviser's toolkit." Silence creates tension, signals emotional weight, allows the audience to process, gives the performer time for a genuine choice, and communicates volumes about inner state. The most underused tool in improv.

## Macro-pacing (across a show)

## Scene length variety

A show where every scene is the same length becomes monotonous regardless of content. Vary deliberately: long scenes (5-8 min) for deep relationship work, medium (2-4 min) for game heightening, short (30 sec - 1 min) for callbacks and punctuation.

![Three scene lengths drawn to scale against what each is for: long for relationship work, medium for heightening, and short for callbacks.](/images/length-is-a-choice.svg)

*Which makes the running order a design problem rather than a matter of what happens next.*

## Alternating energy levels

The "peaks and valleys" principle. The ensemble senses: "We've been high-energy for three scenes — time for something grounded." This is often the editor's job, but in self-edited forms the whole ensemble shares it.

## Editing IS pacing at the show level

A quick edit signals "we're moving fast." A patient hold signals "this moment matters." The best editors have a musical sense — they feel when a scene has peaked and cut at the top.

Jimmy Carrane: "The edit is the most important move in long-form improv. It's where the show's rhythm lives."

## Specific sources

Napier, *Behind the Scenes: Improvising Long Form* (2015) — the best published source on show-level pacing, covering "finding the dominant energy" and running order. Carrane, ["3 Ways to Get Better at Editing"](https://jimmycarrane.com/3-ways-to-get-better-at-editing-improv-scenes/) — editing as pacing. **Attribution note:** The Del Close quote "the pause is the most powerful tool" is widely attributed but has no verified published source — it circulates as oral tradition from iO teaching. The Carrane quotes on editing as rhythm are consistent with his philosophy but exact wordings are from classes, not publications.