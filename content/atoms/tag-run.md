---
id: tag-run
title: Tag Run
type: technique
status: draft
tags: [techniques, editing, harold, game, advanced]
aliases: ["Tag-out"]
links:
  - { id: editing, relation: extends }
  - { id: game-of-the-scene, relation: extends }
  - { id: heightening, relation: extends }
  - { id: if-this-then-what, relation: illustrates }
  - { id: support-moves, relation: extends }
  - { id: harold, relation: extends }
  - { id: ensemble, relation: requires }
  - { id: active-listening, relation: requires }
  - { id: commitment, relation: requires }
  - { id: be-simple, relation: requires }
  - { id: obvious-choice, relation: requires }
  - { id: ref-ucb-manual, relation: extends }
sources: []
created: "2026-04-05"
updated: "2026-08-24"
---

A rapid sequence of tag-outs exploring one game or pattern across multiple contexts. One player (or the game itself) remains constant while scene partners are tagged out and replaced in quick succession. Each new scene transplants the same pattern into a new scenario, heightening with each iteration.

## How it works

1. A clear game is identified in a scene (e.g., a character who gives wildly inappropriate advice)
2. A backline player tags out one performer, starting a new scene with the remaining player
3. The game player exhibits the same pattern in the new context (now giving inappropriate advice to a doctor, a judge, a child)
4. Another tag — new context, same game, escalated
5. Repeat 3-5+ times, each scene lasting 10-30 seconds

The result is a machine-gun exploration of if-this-then-what: "If this person gives bad advice HERE, what happens when they give it THERE?"

## How tag runs differ from single tag-outs

- A **single tag-out** is one edit — it shifts the scene to a new context
- A **tag run** is a sustained *series* that becomes a structure unto itself, generating its own momentum and energy
- Tag runs have escalation built in — each new context should be more extreme, more surprising, or more revealing than the last

## When to use tag runs

- When a game is so clear and simple that it can be instantly read in any new context
- When the ensemble collectively recognizes the game and wants to explore it
- When the scene's game has more to give than a single scene can contain
- When you want to end a Harold beat with explosive energy

## When NOT to use tag runs

- When the game is too complex to read in 10 seconds (mapping, nuanced relationship games)
- When the scene is emotional/relationship-driven — tag runs interrupt intimacy
- When only one person on the backline sees the game — the run will stall

## The discipline

Each tag must be committed and specific — a vague context wastes the audience's calibration. The game player must commit fully to each new reality while maintaining the core pattern. The backline must be listening and ready with the next context before they enter. Speed without clarity is chaos; clarity without speed is just a series of tag-outs.

## Specific sources

UCB Manual, Ch. 6-7 (Edits / The Harold) — tag-outs and tag runs as Harold editing techniques. Tag runs are primarily performance/oral tradition — much of the pedagogy lives in advanced UCB/iO curricula rather than in books. **Counter-position:** Tag runs are a game-first structure. The TJ & Dave tradition would never use one — their organic, relationship-driven scenes don't break for rapid-fire contextual exploration. Tag runs are a tool of the game tradition, not a universal improv technique.