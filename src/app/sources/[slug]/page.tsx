...
import { loadSources, getSourceBySlug, getAtomBySlug } from "@/lib/content";
...
export default async function SourcePage({
  ...
  const atoms = await Promise.all(
    (fm.atoms_extracted ?? []).map(async (id) => {
      ...
      return atom
        ? { id, title: atom.frontmatter.title, type: atom.frontmatter.type, found: true }
        : { id, title: id, type: "unknown", found: false };
    })
  );
