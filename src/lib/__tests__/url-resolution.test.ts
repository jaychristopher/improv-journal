...
describe("getAtomUrl", () => {
  ...
  it("maps law atoms to /how-it-works/{id}", () => {
    expect(getAtomUrl({ id: "irreversibility", type: "law" })).toBe(
      "/how-it-works/irreversibility",
    );
    expect(getAtomUrl({ id: "cognitive-bandwidth", type: "law" })).toBe(
  ...
  it("maps framework atoms to /how-it-works/diagnosis/{id}", () => {
    expect(getAtomUrl({ id: "systemic-collapse-modes", type: "framework" })).toBe(
      "/how-it-works/diagnosis/systemic-collapse-modes",
    );
  });
  ...
  it("maps exercise atoms to /practice/exercises/{id}", () => {
    expect(getAtomUrl({ id: "mirroring", type: "exercise" })).toBe("/practice/exercises/mirroring");
  });

  it("maps technique atoms to /practice/techniques/{id}", () => {
    expect(getAtomUrl({ id: "yes-and", type: "technique" })).toBe("/practice/techniques/yes-and");
  ...
  it("maps format atoms to /practice/formats/{id}", () => {
    expect(getAtomUrl({ id: "harold", type: "format" })).toBe("/practice/formats/harold");
    expect(getAtomUrl({ id: "armando", type: "format" })).toBe("/practice/formats/armando");
  });

  it("maps definition atoms to /practice/vocabulary/{id}", () => {
    expect(getAtomUrl({ id: "offers", type: "definition" })).toBe("/practice/vocabulary/offers");
  ...
  it("handles every AtomType without falling to default", () => {
    ...
    for (const type of types) {
      ...
      expect(url).toMatch(/^\/(how-it-works|practice|library)\//);
    }
  });
